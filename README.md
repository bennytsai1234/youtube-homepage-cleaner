...
*   **v1.6.0 (重大更新)**:
    ...
    *   ⚡ **極致效能**: 導入 `requestIdleCallback` 與智慧防抖技術，滑動瀏覽更流暢，不卡頓。
    *   🌐 **國際化支援 (i18n)**: 完整支援**繁體中文、简体中文、English、日本語**，包含介面與數字單位自動換算 (如 1.2萬、50K)。
    *   📊 **過濾統計**: 新增可視化統計面板，清楚知道幫您擋掉了多少垃圾內容。
    *   💾 **設定匯出/匯入**: 換電腦或重裝不用怕，設定一鍵備份。
    *   🛡️ **Anti-Adblock 2.0**: 全新白名單機制，精準攔截廣告彈窗的同時，不再誤殺會員或檢舉視窗。
*   **v1.5.7**:
